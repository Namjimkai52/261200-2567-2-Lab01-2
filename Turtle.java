...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a) {
        ...
        // 1. ตั้งค่าสำคัญ
        t.speed(0.1); // **สำคัญ** ต้องปรับให้เร็วสุดๆ ไม่งั้นรอนานมากครับ
        t.penColor("deepskyblue"); // สีฟ้าเข้มสไตล์น้ำแข็ง (ลองเปลี่ยนเป็น "cyan" ก็ได้)
        t.width(2); // เส้นบางๆ จะได้ดูพริ้วไหว

        // เราจะวาดเกล็ดหิมะที่มี 6 แฉกหลัก
        // โดยการวาดกิ่ง 1 กิ่ง แล้วหมุน 60 องศา ทำซ้ำ 6 รอบ

        for (int i = 0; i < 6; i++) {

            // ============ เริ่มวาดกิ่งหลัก 1 กิ่ง ============

            // --- ช่วงโคนกิ่ง (ใหญ่สุด) ---
            t.forward(40);

            // กิ่งย่อยแยกซ้ายขวา (ชุดที่ 1)
            t.left(45);   t.forward(30); t.backward(30); // กิ่งซ้าย
            t.right(90);  t.forward(30); t.backward(30); // กิ่งขวา
            t.left(45);   // หันกลับมาตรงกลาง

            // --- ช่วงกลางกิ่ง ---
            t.forward(30);

            // กิ่งย่อยแยกซ้ายขวา (ชุดที่ 2 - เล็กลง)
            t.left(45);   t.forward(20); t.backward(20); // กิ่งซ้าย
            t.right(90);  t.forward(20); t.backward(20); // กิ่งขวา
            t.left(45);   // หันกลับมาตรงกลาง

            // --- ช่วงปลายกิ่ง (เล็กสุด) ---
            t.forward(20);

            // กิ่งย่อยที่ปลายยอด (ชุดที่ 3)
            t.left(45);   t.forward(15); t.backward(15); // กิ่งซ้าย
            t.right(90);  t.forward(15); t.backward(15); // กิ่งขวา
            t.left(45);   // หันกลับมาตรงกลาง
            ...
            // *** สำคัญมาก: ถอยหลังกลับไปที่จุดศูนย์กลาง (0,0) ***
            // ระยะทางรวมที่เราเดินหน้ามาคือ 40 + 30 + 20 = 90
            t.backward(90);

            // ============ จบ 1 กิ่ง ============

            // หมุน 60 องศา เพื่อเตรียมวาดกิ่งต่อไป (360 / 6 แฉก = 60)
            t.right(60);
        }

        // จบงาน: ยกปากกาแล้วหลบฉากไป
        t.up();
        t.forward(250);
    }
